...
| Item | Onde |
|------|------|
| Lease no orquestrador | `015_outbound_dispatch_lease.sql`; nó **Claim dispatch lease** em [`sdr-outbound-workflow.json`](../workflows/sdr-outbound-workflow.json) |
...
2. **Agente Outbound v2:** antes de enviar WA, validar sessão (ex.: não competir com lease do orquestrador ou exigir `status`/pausa alinhados ao questionário). *Depende de E.1.3–E.1.5.*
